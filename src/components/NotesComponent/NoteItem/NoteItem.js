  ...
  return (
    ...
    >
      <div className="wrapper" style={{ textAlign: `${props.type === "right" ? "right" : "left"}` }}>
        <div
          className="note_time"
          style={{ flexDirection: `${data && data.email === user?.email ? "row-reverse" : "none"}` }}
        >
          <img
            className="note_avatar"
            src={`${data && data.author && data.author.image ? data.author.image : userAvatar}`}
            alt="avatar-user"
          />
          {formatDate(data.timeCreate)}
        </div>
